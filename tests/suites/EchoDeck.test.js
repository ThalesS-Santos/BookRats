...
describe('EchoDeck Component', () => {
  ...
  it('handles PanResponder callbacks (swipe and tap)', () => {
    ...
    expect(handlers.onMoveShouldSetPanResponder(null, { dx: 15, dy: 0 })).toBe(
      true,
    );
    expect(handlers.onMoveShouldSetPanResponder(null, { dx: 5, dy: 0 })).toBe(
      false,
    );
    expect(handlers.onMoveShouldSetPanResponder(null, { dx: 15, dy: 20 })).toBe(
      false,
    );
  });
