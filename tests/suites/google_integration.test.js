...
describe('Google Books Integration (via MSW)', () => {
  ...
  it('should fetch and normalize "Dom Casmurro" correctly with all fields (Step 1.8)', async () => {
    ...
    expect(book.thumbnail).toEqual({ uri: 'https://example.com/dom_casmurro.jpg' }); // Note: MSW returns http, service forces https
  });
  ...
  it('should handle 500 Internal Server Error via trigger keyword', async () => {
    await expect(searchBooks({ generalQuery: 'trigger_error_500' }))
      .rejects.toThrow('Erro interno no servidor remoto. Tente novamente mais tarde.');
    
    expect(Logger.error).toHaveBeenCalled();
  });

  it('should handle 429 Rate Limit Error via trigger keyword', async () => {
    await expect(searchBooks({ generalQuery: 'trigger_rate_limit' }))
