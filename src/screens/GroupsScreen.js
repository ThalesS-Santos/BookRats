...
export default function GroupsScreen() {
  ...
  useEffect(() => {
    const unsubMessages = subscribeToGroupMessages('squad-geral');
    return () => {
  ...
  const handleSendMessage = () => {
    ...
    sendMessage('squad-geral', messageText.trim());
    setMessageText('');
  };
  ...
  return (
    <View className="flex-1 bg-background-light dark:bg-background-dark px-6 pt-4">
      <View className="mb-4">
        ...
        <Text className="text-text-light dark:text-text-dark text-3xl font-serif font-bold">Chat Geral</Text>
      </View>
      ...
      <View className="flex-row items-center p-2 bg-card-light dark:bg-card-dark rounded-2xl border border-border-light dark:border-border-dark mb-4">
        <TextInput
