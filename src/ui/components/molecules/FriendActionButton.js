 ...
 */
export default function FriendActionButton({ targetUserId, initialStatus = 'none', containerStyle }) {
  ...
  const hasSentRequest = sentRequests.some(r => r.receiverId === targetUserId && r.status === 'pending');
  
  const status = isFriend ? 'accepted' : (hasSentRequest ? 'pending' : initialStatus);
  ...
  if (status === 'accepted') {
    return (
      <View className="flex-row items-center bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-full border border-gray-200 dark:border-gray-700">
        ...
        <Text className="ml-1.5 text-gray-600 dark:text-gray-300 font-medium text-xs">Amigos</Text>
      </View>
    );
  }
  ...
  if (status === 'pending') {
    return (
      <View className="flex-row items-center bg-gray-50 dark:bg-gray-900 px-3 py-1.5 rounded-full border border-dashed border-gray-300 dark:border-gray-600">
        ...
        <Text className="ml-1.5 text-gray-400 dark:text-gray-500 font-medium text-xs">Pendente</Text>
      </View>
    );
  }
