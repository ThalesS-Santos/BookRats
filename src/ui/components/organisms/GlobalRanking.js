...
const RankingRow = React.memo(({ user, index, isCurrentUser }) => {
  ...
  return (
    <View
      ...
      {/* Position */}
      <View className="w-12 items-center justify-center">{getMedal()}</View>
      ...
      <View className="flex-1 flex-row items-center ml-2">
        ...
        <View className="ml-3 flex-1">
          ...
          <Text
            ...
            @
            {user.username ||
              UserNormalizationService.normalizeDisplayName(user).toLowerCase()}
          </Text>
        </View>
      </View>
      ...
      <View className="w-20 items-center">
        ...
        <Text className="text-gray-400 dark:text-gray-500 text-[10px] uppercase">
          {'Páginas'}
        </Text>
      </View>
  ...
  const renderHeader = () => (
    <View>
      ...
      <View className="flex-row p-4 bg-white dark:bg-background-dark">
        <View className="flex-row flex-1 bg-gray-100 dark:bg-gray-800 rounded-2xl p-1">
          <TouchableOpacity
            ...
            <Text
              className={`font-bold ${activeScope === 'global' ? 'text-primary-light dark:text-primary-dark' : 'text-gray-500'}`}>
              {'Global'}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            ...
            <Text
              className={`font-bold ${activeScope === 'amigos' ? 'text-primary-light dark:text-primary-dark' : 'text-gray-500'}`}>
              {'Amigos'}
            </Text>
          </TouchableOpacity>
        </View>
      ...
      <View className="flex-row items-center px-4 py-3 bg-gray-50 dark:bg-gray-800 border-b border-border-light dark:border-border-dark">
        <Text className="w-12 text-center text-[10px] font-bold text-gray-400 uppercase">
          {'#'}
        </Text>
        <Text className="flex-1 ml-2 text-[10px] font-bold text-gray-400 uppercase">
          {'Leitor'}
        </Text>
        <Text className="w-20 text-center text-[10px] font-bold text-gray-400 uppercase">
          {'Páginas'}
        </Text>
        <Text className="w-16 text-center text-[10px] font-bold text-gray-400 uppercase">
          {'Interação'}
        </Text>
      </View>
    </View>
