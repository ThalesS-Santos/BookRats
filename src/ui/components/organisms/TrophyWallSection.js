...
export default function TrophyWallSection({
  ...
  return (
    <View className="mb-10 bg-card-light dark:bg-card-dark p-4 rounded-2xl border border-border-light dark:border-border-dark -mt-2">
      {/* Filter Pills */}
      <View className="flex-row justify-between mb-4 bg-background-light dark:bg-background-dark p-1.5 rounded-2xl border border-border-light dark:border-border-dark">
        {FILTER_TABS.map(tab => {
          ...
          return (
            <TouchableOpacity
              ...
              <Text
                className={`text-[10px] font-bold ${isActive ? 'text-white dark:text-background-dark font-serif' : 'text-text-muted-light dark:text-text-muted-dark'}`}>
                {tab.label}
              </Text>
            </TouchableOpacity>
          );
            ...
            <TouchableOpacity
              ...
              className={`p-3 rounded-xl border items-center mb-2 w-[31%] ${
                badge.isUnlocked
                  ...
                  : 'bg-card-light dark:bg-card-dark border-dashed border-border-light dark:border-border-dark opacity-40'
              }`}>
      ...
      <View className="flex-row justify-between mt-2">
        ...
        {badgeLimit > 9 && (
          <TouchableOpacity
            ...
            className="flex-1 p-4 rounded-xl border border-text-muted-light/30 dark:border-text-muted-dark/30 bg-text-muted-light/5 dark:bg-text-muted-dark/5 flex-row justify-center items-center ml-1">
            <Ionicons
              name="chevron-up"
              size={16}
              color={
