    ...
    >
      ...
      >
        <View className="flex-row justify-between items-center mt-8 mb-6">
          ...
          >
            <Ionicons name="options-outline" size={18} color={showFilters ? 'white' : accentColor} />
            <Text className={`ml-2 font-bold text-xs ${showFilters ? 'text-white' : 'text-text-light dark:text-text-dark'}`}>
              FILTROS {filters.subjects.length > 0 ? `(${filters.subjects.length})` : ''}
            </Text>
          </TouchableOpacity>
        </View>

        <Text className="text-text-light dark:text-text-dark text-4xl font-serif font-bold mb-2">Descobrir</Text>
        <Text className="text-text-muted-light dark:text-text-muted-dark text-lg font-serif italic mb-6">Encontre sua próxima grande aventura.</Text>
        ...
        {showFilters && (
          <View className="bg-card-light dark:bg-card-dark rounded-3xl p-5 mb-6 border border-border-light dark:border-border-dark shadow-md">
            ...
            <View className="flex-row justify-between items-center mb-3 ml-1">
              <Text className="text-text-light dark:text-text-dark font-bold text-[10px] uppercase tracking-widest">Autor Específico</Text>
              {filters.author.length > 0 && (
                <TouchableOpacity onPress={() => updateFilters({ author: '' })}>
                  <Text className="text-primary text-[10px] font-bold">LIMPAR</Text>
                </TouchableOpacity>
              )}
            </View>
            ...
            <View className="flex-row justify-between mb-6">
              <View className="flex-1 mr-2">
                <Text className="text-text-light dark:text-text-dark font-bold text-[10px] uppercase tracking-widest mb-3 ml-1">Tipo de Conteúdo</Text>
                <View className="flex-row bg-background-light dark:bg-background-dark rounded-2xl p-1 border border-border-light/50 dark:border-border-dark/50">
                  ...
                  >
                    <Text className={`text-[10px] font-bold ${filters.printType === 'all' ? 'text-white' : 'text-text-muted-light dark:text-text-muted-dark'}`}>TUDO</Text>
                  </TouchableOpacity>
                  <TouchableOpacity 
                    onPress={() => updateFilters({ printType: 'books' })}
                    className={`flex-1 py-3 items-center rounded-xl ${filters.printType === 'books' ? 'bg-primary' : ''}`}
                  >
                    <Text className={`text-[10px] font-bold ${filters.printType === 'books' ? 'text-white' : 'text-text-muted-light dark:text-text-muted-dark'}`}>LIVROS</Text>
                  </TouchableOpacity>
                </View>
              </View>

              <View className="flex-1 ml-2">
                <Text className="text-text-light dark:text-text-dark font-bold text-[10px] uppercase tracking-widest mb-3 ml-1">Ordenação API</Text>
                <View className="flex-row bg-background-light dark:bg-background-dark rounded-2xl p-1 border border-border-light/50 dark:border-border-dark/50">
                  <TouchableOpacity 
                  ...
                  >
                    <Text className={`text-[10px] font-bold ${filters.orderBy === 'relevance' ? 'text-white' : 'text-text-muted-light dark:text-text-muted-dark'}`}>RELEV.</Text>
                  </TouchableOpacity>
                  <TouchableOpacity 
                    onPress={() => updateFilters({ orderBy: 'newest' })}
                    className={`flex-1 py-3 items-center rounded-xl ${filters.orderBy === 'newest' ? 'bg-primary' : ''}`}
                  >
                    <Text className={`text-[10px] font-bold ${filters.orderBy === 'newest' ? 'text-white' : 'text-text-muted-light dark:text-text-muted-dark'}`}>NOVO</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </View>
            ...
            {/* Subjects Scroll */}
            <View className="flex-row justify-between items-center mb-3 ml-1">
              <Text className="text-text-light dark:text-text-dark font-bold text-[10px] uppercase tracking-widest">Categorias (Multi-seleção)</Text>
              {filters.subjects.length > 0 && (
                <TouchableOpacity onPress={() => updateFilters({ subjects: [] })}>
                  <Text className="text-primary text-[10px] font-bold">LIMPAR TUDO</Text>
                </TouchableOpacity>
              )}
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row" contentContainerStyle={{ paddingBottom: 5 }}>
              {SUBJECTS.map((s) => {
                ...
                return (
                  ...
                  >
                    <Ionicons name={s.icon} size={16} color={isActive ? 'white' : accentColor} />
                    <Text className={`ml-2 text-xs font-bold ${isActive ? 'text-white' : 'text-text-light dark:text-text-dark'}`}>
                      {s.label}
                    </Text>
                  </TouchableOpacity>
                );
        ...
        <View className="mb-2">
          <View className="flex-row items-center bg-card-light dark:bg-card-dark rounded-2xl border-2 border-primary/20 dark:border-primary-dark/20 px-4 shadow-sm">
            ...
            <TextInput
              ...
              placeholder={filters.author ? `Obras de ${filters.author}...` : "O que vamos ler hoje?"}
              placeholderTextColor={mutedTextColor}
            ...
            {(query.length > 0 || filters.author.length > 0 || filters.subjects.length > 0) && !loading && (
              <TouchableOpacity onPress={clearSearch}>
                <Ionicons name="close-circle" size={22} color={mutedTextColor} />
              </TouchableOpacity>
            )}
          </View>
        </View>
