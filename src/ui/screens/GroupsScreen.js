      ...
      <Animated.View
        ...
        <TouchableOpacity
          ...
          <View className="flex-row items-center flex-1 pr-4">
            ...
            <View className="flex-1">
              <Text
                className="text-text-light dark:text-text-dark font-serif font-bold text-lg"
                numberOfLines={1}>
                {item.name}
              </Text>
      ...
      <Animated.View
        ...
        <TouchableOpacity
          ...
          <View className="flex-row items-center flex-1 pr-4">
            ...
            <View className="flex-1">
              <Text
                className="text-text-light dark:text-text-dark font-bold text-lg"
                numberOfLines={1}>
...
export default function GroupsScreen({ navigation }) {
  ...
  const groupsData =
    (loadingSocial && groups.length === 0) || !isReady
      ? Array(5)
          .fill({})
          .map((_, i) => ({ id: `skeleton-group-${i}`, isSkeleton: true }))
      : groups;

  const friendsData =
    (loadingSocial && friends.length === 0) || !isReady
      ? Array(6)
          .fill({})
          .map((_, i) => ({ id: `skeleton-friend-${i}`, isSkeleton: true }))
      : friends;
      ...
      <Animated.View
        ...
        <View className="flex-row mb-6 bg-card-light dark:bg-card-dark p-1 rounded-xl border border-border-light dark:border-border-dark">
          <TouchableOpacity
            ...
            <Text
              className={`font-bold ${activeTab === 'groups' ? 'text-white' : 'text-text-muted-light dark:text-text-muted-dark'}`}>
              Grupos
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            ...
            <Text
              className={`font-bold ${activeTab === 'friends' ? 'text-white' : 'text-text-muted-light dark:text-text-muted-dark'}`}>
              Amigos
            </Text>
          </TouchableOpacity>
        </View>
      ...
      {activeTab === 'groups' && (
        <View className="flex-1">
          ...
          <FlatList
            ...
            ListEmptyComponent={
              !loadingSocial ? (
                <Text className="text-text-muted-light dark:text-text-muted-dark text-center mt-10">
      ...
      <Modal visible={modalVisible} animationType="slide" transparent={true}>
        <View className="flex-1 justify-end bg-black/50">
          <View className="bg-background-light dark:bg-background-dark p-6 rounded-t-3xl border-t border-border-light dark:border-border-dark h-[80%]">
            ...
            <Text className="text-text-light dark:text-text-dark font-bold mb-2">
              Nome do Grupo
            </Text>
            <TextInput
            ...
            <ScrollView className="flex-1 mb-4">
              {friends.length === 0 ? (
                <Text className="text-text-muted-light dark:text-text-muted-dark text-sm">
                  Você precisa de amigos para criar um grupo.
                </Text>
              ) : (
                friends.map(friend => (
                  <TouchableOpacity
