...
describe('Auth Integration Flow', () => {
  beforeEach(() => {
    ...
    useBookStore.setState({ loading: false, user: null, authError: null });
  });

  it('Scenario 1: should trigger successful login flow when credentials are valid', async () => {
    const { getByPlaceholderText, getByText } = render(<AuthScreen />, { wrapper: componentWrapper });
  ...
  it('Scenario 2: should hide "Entrar" text and show loading indicator during authentication', async () => {
    const { getByPlaceholderText, getByText, queryByText } = render(<AuthScreen />, { wrapper: componentWrapper });
  ...
  it('Scenario 3: should display CustomPopup with the safe mapped error message on authentication failure', async () => {
    ...
    const { getByPlaceholderText, getByText, findByText } = render(<AuthScreen />, { wrapper: componentWrapper });
  ...
  it('UX Validation: should not trigger API call with invalid email or short password', async () => {
    ...
    fireEvent.changeText(getByPlaceholderText('seu@email.com'), 'test@valid.com');
