...
describe('Error Mapper Utility', () => {
  ...
  describe('Auth Error Mapping', () => {
    test.each(authCases)('should map %s to Brazilian Portuguese', (code, expected) => {
      expect(mapFirebaseError({ code })).toBe(expected);
    });
  });

  describe('Firestore Error Mapping', () => {
    test.each(firestoreCases)('should map %s to Brazilian Portuguese', (code, expected) => {
      expect(mapFirebaseError({ code })).toBe(expected);
    });
  });

  describe('Quota & Other Error Mapping', () => {
    test.each(quotaCases)('should map %s to Brazilian Portuguese', (code, expected) => {
      expect(mapFirebaseError({ code })).toBe(expected);
    });
  });
  ...
  describe('Robustness & Input Validation', () => {
    it('should handle null input gracefully', () => {
      expect(mapFirebaseError(null)).toBe('Ocorreu um erro inesperado. Tente novamente.');
    });

    it('should handle undefined input gracefully', () => {
      expect(mapFirebaseError(undefined)).toBe('Ocorreu um erro inesperado. Tente novamente.');
    });

    it('should handle an empty object input gracefully', () => {
      expect(mapFirebaseError({})).toBe('Ocorreu um erro inesperado. Tente novamente.');
    });

    it('should handle input without a code property', () => {
      expect(mapFirebaseError({ message: 'Something went wrong' })).toBe('Ocorreu um erro inesperado. Tente novamente.');
    });
  });
});
